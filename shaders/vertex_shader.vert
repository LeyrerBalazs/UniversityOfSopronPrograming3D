...
void main() {
   ...
   vec3 normal = normalize( mat3(transpose(inverse(modelView))) * v_normal_color);
   vec3 lightDir = normalize( vec4(lightPos, 1.0) - world_position  ).xyz;
   float diffColor = max(dot(normal, lightDir), 0.0) * 0.7;
   color = v_normal_color + diffColor;
}
